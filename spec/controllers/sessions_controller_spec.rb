...
describe SessionsController do
  ...
  before do 
    ...
    Member.stub!(:authenticate).with(@login_params[:login], @login_params[:password]).and_return(@member)
  end
  def do_create
          ...
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            ...
            if (has_request_token == :valid)
              ...
              if want_remember_me
                ...
                it 'does not make new token' do @member.should_not_receive(:remember_me);   do_create end
                it 'does not refresh token'  do @member.should_not_receive(:refresh_token); do_create end 
                it 'kills user token'        do @member.should_receive(:forget_me);         do_create end 
              end
            end
          end # inner describe
        end
...
describe SessionsController do
  ...
  describe "named routing" do
    ...
    it "should route sessions_path() correctly" do
